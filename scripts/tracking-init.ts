...
import {
  ...
  quoteIdent,
  renderScript,
} from '../server/utils/trackingSchema.ts';
import { classifyDbError, poolOptions, STATUS_HINTS } from '../server/utils/trackingDb.ts';

/* Minimal .env reader. Nuxt loads `.env` itself at dev/build time, but this
   script runs as a plain Node process outside that, so it has to do it here. */
