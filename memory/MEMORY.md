...
- [Seed-auth plugin Pinia ordering](project-seed-auth-pinia-ordering.md) — `seed-auth.server.ts` MUST use `dependsOn: ['pinia']`, NOT `enforce: 'pre'`; otherwise SSR crashes with downstream SyntaxError + `$setup.X is not a function` symptoms
- [script-setup await binding bug](feedback-script-setup-await-binding.md) — `function X()` after top-level `await useFetch` can fail to expose on `$setup`; use `const X = computed(() => …)` for derived values rendered in templates
- [i18n locales registry](project-i18n-locales-registry.md) — `app/locales/_registry.ts` is auto-generated by `scripts/build-locales-registry.mjs`; `predev`/`prebuild` npm hooks regenerate it; check-in to git but DO NOT hand-edit
- [AccessErrorView wiring](project-access-error-view.md) — friendly 403/404 card on 4 detail pages; `classifyApiError()` string-matches the SDK message; `backLabel` needs `computed(() => labels.value.X)` deref
- [Header auth-request must clear cart](feedback-header-handleAfterRequestAuthorization.md) — `AppHeader.handleAfterRequestAuthorization` MUST call `setCart(restoreManagerCart())`; the package's CartSummary fires the callback wherever it's mounted
